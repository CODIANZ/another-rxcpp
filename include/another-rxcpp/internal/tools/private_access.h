...
class observable {
public:
  template <typename OBSERVABLE> static auto create_source(const OBSERVABLE& o) {
...
class subscriber {
public:
  template <typename SUBSCRIBER, typename UPSTREAM> static void add_upstream(const SUBSCRIBER& s, UPSTREAM upstream) {
    s.add_upstream(std::forward<UPSTREAM>(upstream));
  }
  template <typename SUBSCRIBER> static void unsubscribe_upstreams(const SUBSCRIBER& s) {
